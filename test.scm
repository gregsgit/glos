,open cells tables define-record-types records record-types srfi-1 srfi-8 srfi-23 formats ports fluids closures
,load utils.scm
,load callable-macros.scm tlet-macro.scm util-macros.scm 
,load glos-records.scm types.scm callables.scm
,load runtime-library.scm
  ...
  (syntax-rules ()
    ((test2 e1 e2)
     (let ((v1 e1)
       ...
       (if (not (equal? v1 v2))
	   (begin
	     (format #t "~%Test failed: ~a -> ~a not eq ~a -> ~a ~%" 'e1 v1 'e2 v2)
	     #f)
	   (begin
	     (format #t ".")
	     #t))))))
...
(defmethod (m1 (i <integer>))
  (format #t "m1<integer>(~a)~%" i)
  i)

(defmethod (m2 (i <integer>) (s string?))
  (format #t "m2, i=~a, s=~a~%" i s)
  (list i s))
...
(defgeneric g1 m1
  (method ((n <number>))
	  (format #t "g1<number>(~a)~%" n)
	  n)
  (method ((s string?))
	  (format #t "g1string?(~a)~%" s)
	  s))
...
(add-method
 ...
 (method ((x (and? <integer> even?)))
	 (format #t "g1 on even int, (~a)~%" x)
	 (call-next-method)))

(replace-method
 g1 (make-signature-type #f <integer>)
 (method ((i <integer>))
	 (format #t "new g1<int>(~a)~%" i)
	 (call-next-method)))
...
(defmethod (bm1 x)
  (format #t "in bm1, got ~a~%" x))

(defmethod (bm2 (x <int>))
  (format #t "in bm2 <int>, got ~a~%" x))
...
(add-after-method
 g1 (method ((x string?))
	    (format #t "in after method on string?, retval=~a~%"
		    (fluid *return-value*))))
...
(add-after-method
 g1 (method (x)
	    (format #t "in after method on <top>~%")))
...
(add-around-method
 g1
 (method ((x <int>))
	 (format #t "in around method on <int>, calling next-method~%")
	 (call-next-method)))

(add-around-method
 g1
 (method ((x <string>))
	 (format #t "in around method on <string>, returning 'foo~%")
	 'foo))

(add-around-method
 g1
 (method ((x <number>))
	 (format #t "in around method on <number>, returning 'bar~%")
	 'bar))

(add-around-method
 g1
 (method ((x <string>))
	 (format #t "in around method on <string>, calling next method~%")
	 (let ((val (call-next-method)))
	   (format #t "back in around method on <string>, got ~a~%" val)
	   val)))
...
(gfmethod (bar (x (and? <top> (fluid *in-foo?*))))
	  (format #t "in bar, in cflow of foo~%"))
(gfmethod (bar (x <top>))
	  (format #t "in bar, not in cflow of foo~%"))
...
(defgeneric walk-node
  (method ((n <leaf-node>))
	  (format #t "leaf node ~a~%" (node-data n)))
  (method ((n <interior-node>))
	  (format #t "interior node ~a~%" (node-data n))
...
(add-before-method
 move (method ((p <colorpoint>) (x <integer>) (y <integer>))
	      (format #t "before move colorpoint (~a, ~a)~%" x y)))

(add-after-method
 move (method ((p <point>) (x <integer>) (y <integer>))
	      (format #t "after move point (~a, ~a)~%" x y)))
...
(add-after-method
 move (method ((p <colorpoint>) (x (and? <integer> even?)) (y <integer>))
	      (format #t "after move cp to even x~%")))

(defgeneric g2
  (method ((o <number>))
	  (format #t "g1 on number~%"))
  (method ((o <integer>))
	  (format #t "g1 on integer~%")
	  (call-next-method)))
(g2 2)
...
(defgeneric show
  (method ((o <wall>)) (format #t "<wall>~%"))
  (method ((o <room>)) (format #t "<room> ~a~%" (get-n o)))
  (method ((o <bombed-wall>)) (format #t "<bombed-wall>~%"))
  (method ((o <bombed-room>))(format #t "<bombed-room> ~a~%" (get-n o))))
...
(define c1 (new <chassis> 'name 'chassis1 'price 1
		'parts (list f1 f2)))
...
(defmethod (draw-border w)
  (format #t "drawing border of width ~a~%" w))

(defmethod (draw-scroll-bar)
  (format #t "drawing scrollbar~%"))

(defgeneric draw
  (method ((comp <visual-component>))
	  (format #t "drawing <visual-component>~%"))
  (method ((w <window>))
	  (draw (window-contents w)))
  (method ((c <text-view>))
	  (format #t "drawing <text-view>~%")))
...
(define c1 (new <character> #\a))
(define c2 (new <character> #\a))
(eq? c1 c2)
...
(defmethod (bits-from-file (fn <string>))
  (format #t "Reading bits from file ~a~%" fn)
  'some-bits)
...
(defgeneric draw
  (method ((i <image>))
	  (let ((bits (image-bits i)))
	    (format #t "Drawing image bits ~a~%" bits))))
...
(gfmethod (handle-help (topic <topic>) (handler <application>))
	  (format #t "Displaying list of application help topics.~%"))
...
(define app1 (new <application>
		  'handler (method ((req <help-request>))
				   (format #t "application help~%"))))

(define d1 (new <dialog> app1
		'handler (method ((req <help-request>))
				 (format #t "dialog help~%"))
		'handler (method ((req <preview-request>))
				 (format #t "dialog preview~%"))))
(define b1 (new <button> d1
		'handler (method ((req <help-request>))
				 (format #t "button help~%"))))
...
(defmethod (set-text (ef <entry-field>) text)
  (format #t "setting text of entry field to ~a.~%" text)
...
(define (call-foo wrapped-foo a-foo . args)
  (format #t "Calling foo[x=~a, y=~a] with args ~a~%" (get-x a-foo) (get-y a-foo) args))
...
(define (foo x)
  (try
   (bar x)
   ((<cond1> (lambda (e) (format #t "got cond1 ~a~%" e)))
    (<cond2> (lambda (e) (format #t "got cond2 ~a~%" e))))
   (format #t "top finally~%")))

(define (bar x)
  (try
   (baz x)
   ()
   (format #t "finally in bar~%")))

(define (baz x)
  (format #t "in baz, throwing cond2:~%")
  (throw <cond2> (list "this is a cond2 exception"))
  (format #t "in baz, shouldn't get here.~%"))
...
(gfmethod (fact (x (eq 3))) (begin (format #t "in fact(3)~%") 6))
(call fact 2)
(call fact 4)
